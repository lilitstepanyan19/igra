...
class World_1_2(WorldBase):
    ...
    def start(self):
        self.sub = self.sub = int(self.__class__.__name__.split("_")[-1]) - 1
        self.levels = [
            {"target": "A", "need": 4, "count": 7, "bg": "images/world_1/bg_2.jpg"},
        ]
        self.load_sublevel()
    ...
    def spawn(self, count):
        ...
        for _ in range(count):
            l = self.target if random.random() < 0.7 else random.choice("ABCDEFGHI")
            x = random.randint(60, WIDTH - 60)
            y = random.randint(140, HEIGHT - 60)
            vx = random.choice([-1, 1]) * 1
            vy = random.choice([-1, 1]) * 1
    ...
    def draw(self, screen):
        ...
        hud = self.game.font_bad.render(
            f"WORLD 1  Level {self.sub +1}  {self.score}/{self.need}", True, (0, 0, 0)
        )
    ...
    def next_world(self):
        from worlds.world_2.world_2_1 import World_2_1

        return World_2_1(self.game)